...
Total `.aspx` pages: **188**. Census status is Reviewed (every page has a catalog row). Domain workflows live after `<!-- NARRATIVE:BEGIN -->` in each `DOMAIN_*.md`. See [SHARED_CONTEXT.md](SHARED_CONTEXT.md) for AuthN/tenancy, [SHARED_RUNTIME.md](SHARED_RUNTIME.md) for handlers/helpers/SPs, [SHARED_SCHEMA.md](SHARED_SCHEMA.md) for live UAT objects, [UAT_CATALOG.md](UAT_CATALOG.md) for every column/SP signature, and [DATA_DICTIONARY.md](DATA_DICTIONARY.md) for page↔table/SP verbs.
