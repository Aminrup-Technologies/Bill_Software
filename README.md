...
| A-21 | **Critical** | Delete client/vendor unscoped concatenated SQL — **Phase 2C** parameterized + `CompanyID` + AuthGuard resource check |
...
| A-28 | **Medium** | Intra-tenant visit IDOR — **Phase 2B** owner/manager-dashboard scope; **Phase 2C** quotation visit attach uses `UserCanViewVisit` |
...
| A-02 | **Critical** | Card kiosk (`tbl_card_login`) — **Phase 3** login SQL parameterized; still plaintext, no ActiveSessions. Isolated from ERP auth. See `docs/21_Phase3_Infrastructure_Hardening.md` |
