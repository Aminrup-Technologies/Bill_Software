        ...
        private string GetItemQueryByDocType(string docType)
        {
            ...
            if (docType == "Proforma")
            {
                ...
                SELECT 
                    ...
                    ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice inv ON id.Invoice_No = inv.Invoice_No AND id.CompanyID = inv.CompanyID WHERE id.Quotation_no = pd.Invoice_No AND id.Product_id = pd.Product_id AND ISNULL(id.ItemNo, '') = ISNULL(pd.ItemNo, '') AND id.CompanyID = @CompanyID AND inv.status2 = 'Active'), 0) AS InvoicedQty,
            ...
            return @"
                SELECT 
                    ...
                    ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice inv ON id.Invoice_No = inv.Invoice_No AND id.CompanyID = inv.CompanyID WHERE id.Quotation_no = cd.Challan_no AND id.Product_id = cd.Product_id AND ISNULL(id.ItemNo, '') = ISNULL(cd.ItemNo, '') AND id.CompanyID = @CompanyID AND inv.status2 = 'Active'), 0) AS InvoicedQty,
