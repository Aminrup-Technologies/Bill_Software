        ...
        private string GetItemQueryByDocType(string docType)
        {
            ...
            if (docType == "Proforma")
            {
                ...
                SELECT 
                    ...
                    ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice iv ON id.Invoice_No = iv.Invoice_No WHERE id.Quotation_no = pd.Invoice_No AND (id.Product_Code = pd.Product_Code OR id.Product_id = pd.Product_id) AND ISNULL(id.ItemNo, '') = ISNULL(pd.ItemNo, '') AND id.CompanyID = @CompanyID AND iv.status2 = 'Active'), 0) AS InvoicedQty,
                    pd.Quantity - ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice iv ON id.Invoice_No = iv.Invoice_No WHERE id.Quotation_no = pd.Invoice_No AND (id.Product_Code = pd.Product_Code OR id.Product_id = pd.Product_id) AND ISNULL(id.ItemNo, '') = ISNULL(pd.ItemNo, '') AND id.CompanyID = @CompanyID AND iv.status2 = 'Active'), 0) AS PendingQty,
            ...
            return @"
                SELECT 
                    ...
                    ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice iv ON id.Invoice_No = iv.Invoice_No WHERE id.Quotation_no = cd.Challan_no AND (id.Product_Code = cd.Product_code OR id.Product_id = cd.Product_id) AND ISNULL(id.ItemNo, '') = ISNULL(cd.ItemNo, '') AND id.CompanyID = @CompanyID AND iv.status2 = 'Active'), 0) AS InvoicedQty,
                    cd.Quantity - ISNULL((SELECT SUM(CAST(id.Quantity AS DECIMAL(18,2))) FROM tbl_Invoice_details id INNER JOIN tbl_Invoice iv ON id.Invoice_No = iv.Invoice_No WHERE id.Quotation_no = cd.Challan_no AND (id.Product_Code = cd.Product_code OR id.Product_id = cd.Product_id) AND ISNULL(id.ItemNo, '') = ISNULL(cd.ItemNo, '') AND id.CompanyID = @CompanyID AND iv.status2 = 'Active'), 0) AS PendingQty,
